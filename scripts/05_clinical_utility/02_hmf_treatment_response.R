...
write_tsv(
  ...
  file.path(outdir, "SF25d_source_data.tsv")
)
...
plot_km(
    ...
    "SF25d_TTF_in_Taxane_vs_ARPI_2nd_line_O-III.pdf"
)
write_tsv(
  drugs_2nd_line %>% dplyr::filter(evolutionary_subtype == "O-II") %>% dplyr::select(TTF, ttf_event, arm),
  file.path(outdir, "SF25c_source_data.tsv")
)
...
plot_km(
    ...
    "",
    "Days from treatment initiation", "Survival rate",
    "SF25c_TTF_in_Taxane_vs_ARPI_2nd_line_O-II.pdf"
)
write_tsv(
  drugs_2nd_line %>% dplyr::filter(evolutionary_subtype == "O-I") %>% dplyr::select(TTF, ttf_event, arm),
  file.path(outdir, "SF25b_source_data.tsv")
)
...
plot_km(
    ...
    "",
    "Days from treatment initiation", "Survival rate",
    "SF25e_TTF_in_Taxane_vs_ARPI_2nd_line_O-I_non_CIN.Cluster-6.pdf"
)
