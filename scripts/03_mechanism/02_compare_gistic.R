...
# Supplementary Table 3, with informative column names. The GISTIC score column
# is -log10(q-value) as emitted by GISTIC (read.delim mangles it to
# X.log10.q.value.); the negation is kept so the header matches the values.
st3 = gistic_scores %>% dplyr::select(
    `Type of SCNA`            = Type,
    `Chromosome`              = Chromosome,
    `Start`                   = Start,
    `End`                     = End,
    `-log10(q-value)`         = X.log10.q.value.,
    `GISTIC score (G.score)`  = G.score,
    `Average amplitude`       = average.amplitude,
    `Frequency`               = frequency,
    `Evolutionary trajectory` = trajectory
)
write_delim(st3, file.path(outdir, "ST_GISTIC_by_trajectory.tsv"), delim = "\t")
...
write_tsv(
  ...
  file.path(outdir, "SF10_amps_source_data.tsv")
)
ggsave(file.path(figdir, "SF10_gistic_amps.pdf"), width = 100/25.3, height = 60/25.3, dpi = 300)
...
write_tsv(
  ...
  file.path(outdir, "SF10_dels_source_data.tsv")
)
ggsave(file.path(figdir, "SF10_gistic_dels.pdf"), width = 100/25.3, height = 60/25.3, dpi = 300)
...
write_tsv(
  ...
  file.path(outdir, "SF11_source_data.tsv")
)
ggsave(file.path(figdir, "SF11_Del_chr_arms_gistic.pdf"), width = 180/25.3, height = 250/25.3, dpi = 300)
...
ggsave(file.path(figdir, "SF12_Amp_chr_arms_gistic.pdf"), width = 180/25.3, height = 250/25.3, dpi = 300)
