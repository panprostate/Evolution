...
# GET TABLE WITH MEAN IMF ACTIVITIES AND WILCOXON P-VALUES FOR HIGHER VS THE OTHER TRAJECTORIES
imf_summary <- imf_activities %>%
    group_by(trajectory, imf_cluster) %>%
    summarise(mean_activity = mean(activity, na.rm = TRUE)) %>%
    pivot_wider(names_from = trajectory, values_from = mean_activity)
...
# Supplementary Table 5, with informative column names. 
# Both the pairwise and the one-versus-rest Wilcoxon
# P values are reported.
st5 = imf_summary %>% dplyr::select(
    `IMF Signature`                          = imf_cluster,
    `Canonical (mean activity)`              = Canonical,
    `Alternative (mean activity)`            = Alternative,
    `Gain-enriched (mean activity)`          = Gain_enriched,
    `P value (Canonical vs Alternative)`     = p_canon_vs_alt,
    `P value (Canonical vs Gain-enriched)`   = p_canon_vs_gain,
    `P value (Alternative vs Gain-enriched)` = p_alt_vs_gain,
    `P value (Canonical vs all others)`      = p_canon_vs_all,
    `P value (Alternative vs all others)`    = p_alt_vs_all,
    `P value (Gain-enriched vs all others)`  = p_gain_vs_all
)
write_delim(st5, file.path(outdir, "ST_IMF_activities_summary.csv"), delim = ",")
...
write_tsv(sigactivities, file.path(outdir, "SF18_source_data.tsv"))
ggsave(
  file.path(figdir, "SF18_Stacked_Signature_Activities_by_Trajectory.pdf"),
  p_sig,
  width = 180/25.4,
  height = 250/25.4,
  units = "in",
...
# GET TABLE WITH MEAN SIGNATURE ACTIVITIES AND WILCOXON P-VALUES FOR HIGHER VS THE OTHER TRAJECTORIES
# Exclude low-prevalence signatures that cannot be tested: a Wilcoxon P value is
# undefined when both compared groups are entirely zero, so require detection
# (activity > 0) in at least two of the three trajectories. This filter applies
# only to the supplementary table below - the figures above use all signatures.
testable_signatures <- sigactivities %>%
    group_by(signature, trajectory) %>%
    summarise(detected = any(activity > 0, na.rm = TRUE), .groups = "drop_last") %>%
    summarise(n_trajectories = sum(detected), .groups = "drop") %>%
    dplyr::filter(n_trajectories >= 2) %>%
    dplyr::pull(signature) %>%
    as.character()

message("ST6: testing ", length(testable_signatures), " of ",
        dplyr::n_distinct(sigactivities$signature), " signatures; excluded: ",
        paste(setdiff(as.character(unique(sigactivities$signature)), testable_signatures),
              collapse = ", "))

sigactivities_tested <- sigactivities %>%
    dplyr::filter(as.character(signature) %in% testable_signatures)

sig_summary <- sigactivities_tested %>%
    group_by(trajectory, signature, sigtype) %>%
    summarise(mean_activity = mean(activity, na.rm = TRUE)) %>%
    pivot_wider(names_from = trajectory, values_from = mean_activity)

wilcox_results_sig <- sigactivities_tested %>%
    group_by(signature) %>%
    summarise(
        p_canon_vs_alt = wilcox.test(activity[trajectory == "Canonical"], activity[trajectory == "Alternative"])$p.value,
        p_canon_vs_gain = wilcox.test(activity[trajectory == "Canonical"], activity[trajectory == "Gain-enriched"])$p.value,
...
sig_summary <- sig_summary %>%
    ...
    dplyr::select(signature, sigtype, Canonical, Alternative, Gain_enriched = `Gain-enriched`, p_canon_vs_alt, p_canon_vs_gain, p_alt_vs_gain, p_canon_vs_all, p_alt_vs_all, p_gain_vs_all) %>%
    dplyr::arrange(sigtype)

# Supplementary Table 6, with informative column names. Activities are means
# (see mean_activity above). Both the pairwise and the one-versus-rest Wilcoxon
# P values are reported.
st6 = sig_summary %>% dplyr::select(
    `Signature`                              = signature,
...
write_delim(st6, file.path(outdir, "ST_signature_activities_summary.csv"), delim = ",")
...
write_tsv(
  ...
  file.path(outdir, "SF19a_source_data.tsv")
)
ggsave(file.path(figdir, "SF19a_sbs_signature_activities_clonal_subclonal.pdf"), width = 180/25.3, height = 50/25.3)
...
write_tsv(
  ...
  file.path(outdir, "SF19b_source_data.tsv")
)
ggsave(file.path(figdir, "SF19b_id_signature_activities_clonal_subclonal.pdf"), width = 180/25.3, height = 50/25.3)
