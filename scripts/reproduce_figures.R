...
#
# Panel order: Main Figures (Fig*) -> Supplementary Figures (SF*). The
# supplementary block is not in strict numeric order: SF1-SF25 is a single
# series renumbered from the former Extended Data + Supplementary sets, and the
# panel sections below are still grouped in their original two-set order.
#
# Schematics (Fig1b, Fig2a, Fig4c, Fig5b, SF25a) and externally-generated
# panels (SF16, SF9a) are skipped; see README.md for the companion repos.
#
...
## plot_timeline_raincloud_tj() — trajectory variant (SF17)
...
## plot_facet() — from scripts/01_landscape/04a_sbs2age.R (SF4)
...
## plot_median_timing() — from scripts/01_landscape/04c_time_landmarks.R (SF6)
...
## boxplot_epicmit() — from scripts/01_landscape/04e_proliferation_markers.R (SF8)
...
## boxplot_rates() — from scripts/04_progression/02a_rate_evolution.R (SF21)
...
## plot_model() — from scripts/04_progression/02c_analysis_simulations.R (Fig4e / SF22)
...
## plot_km() — from scripts/05_clinical_utility/02_hmf_treatment_response.R (SF25b-g)
...
## ggsurvplot wrapper for the Fig5/SF24 KM curves (shared styling)
...
# =============================================================================
# 6. SUPPLEMENTARY FIGURES, part 1 (SF*, former Extended Data set)  -----------
# =============================================================================

# ---- SF1ab  NRPCC vs subclones / min CCF (combined) -----------------------
df <- sd_tsv("SF1ab")
...
save_ggplot(p, fp("SF1ab_nrpcc_power_subclonality"), w = 110, h = 65)
message("Saved: SF1ab")

# ---- SF1c  NRPCC vs subclonal mutation ratio (WCC corrected) --------------
muts_wcc_df <- sd_tsv("SF1c")
p3 <- ggplot(muts_wcc_df,
  ...
  ylim(c(0, 1))
ggsave(fp("SF1c_ratio_subclonal_mutations_wcc_vs_nrpcc.pdf"), p3,
       width = 50/25.3, height = 50/25.3)
message("Saved: SF1c")

# ---- SF3a  dN/dS by Grade Group, clonal vs subclonal ----------------------
dnds_df <- sd_tsv("SF3a")
...
save_ggplot(p, fp("SF3a_dnds_by_gleason_ppcg"), w = 120, h = 60)
message("Saved: SF3a")

# ---- SF3b  SBS signature fold-change clonal vs subclonal by GS group ------
sigtimerk <- sd_tsv("SF3b") %>% dplyr::mutate(grade_group = ifelse(grade_group == "4+", "4-5", grade_group))
...
ggsave(fp("SF3b_sig_change_clonal_subclonal_by_grade_group.png"), p,
       width = 125/25.3, height = 75/25.3, dpi = 300)
ggsave(fp("SF3b_sig_change_clonal_subclonal_by_grade_group.pdf"), p,
       width = 125/25.3, height = 75/25.3, dpi = 300)
message("Saved: SF3b")

# ---- SF13a  UpSet plot of differentially methylated CpGs -------------------
dmcs_wide <- sd_tsv("SF13a") %>% as.data.frame()
...
pdf(fp("SF13a_upset_plot.pdf"), width = 100/25.3, height = 125/25.3)
print(upset_plot); dev.off()
message("Saved: SF13a")

# ---- SF13b  Methylation differences scatter --------------------------------
methylation_differences <- sd_tsv("SF13b")
...
p <- axes2lemon(p, lt = "both", bt = "both")
ggsave(fp("SF13b_methylation_differences_cmp1_cmp2.pdf"), p,
       width = 60/25.3, height = 60/25.3)
message("Saved: SF13b")

# ---- SF13c  DMC CGI annotation distribution --------------------------------
donut_data <- sd_tsv("SF13c") %>%
...
ggsave(fp("SF13c_cpg_annotations.pdf"), donut_plot,
       width = 70/25.4, height = 70/25.4)
message("Saved: SF13c")

# ---- SF13d  DMC distribution across regulatory elements --------------------
reg_elements <- sd_tsv("SF13d")
...
ggsave(fp("SF13d_dmc_regulatory_elements.pdf"), p,
       width = 150/25.3, height = 100/25.3)
message("Saved: SF13d")

# ---- SF14  GSEA bubble plot across trajectories ----------------------------
gsea_plot_df <- sd_tsv("SF14")
p <- ggplot(gsea_plot_df, aes(x = pathway, y = evotype)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
        legend.position = "none")
ggsave(fp("SF14_dea_evotypes.pdf"), plot = p,
       width = 180/25.4, height = 80/25.4)
message("Saved: SF14")

# ---- SF16  Externally generated (trajectory simulation companion repo) -----
# See README.md for the companion repository link.

# ---- SF18  Stacked signature activities per trajectory ---------------------
sigactivities <- sd_tsv("SF18")
...
ggsave(fp("SF18_Stacked_Signature_Activities_by_Trajectory.pdf"),
       p_sig, width = 180/25.4, height = 250/25.4, units = "in", device = cairo_pdf)
message("Saved: SF18")

# ---- SF19a  SBS signature activities, clonal vs subclonal ------------------
sbs_signatures <- sd_tsv("SF19a")
...
p <- axes2lemon(p, lt = "both", bt = "both")
ggsave(fp("SF19a_sbs_signature_activities_clonal_subclonal.pdf"), p,
       width = 180/25.3, height = 50/25.3)
message("Saved: SF19a")

# ---- SF19b  ID signature activities, clonal vs subclonal -------------------
id_signatures <- sd_tsv("SF19b")
p <- id_signatures %>%
  mutate(timing = factor(timing, levels = c("overall", "clonal", "subclonal"))) %>%
  group_by(timing, signature) %>%
  dplyr::filter(n_distinct(trajectory) >= 2,
  ...
  scale_fill_manual(values = trajectory_colours)
p <- axes2lemon(p, lt = "both", bt = "both")
ggsave(fp("SF19b_id_signature_activities_clonal_subclonal.pdf"), p,
       width = 180/25.3, height = 50/25.3)
message("Saved: SF19b")

# ---- SF19c-h  Reptime/size, clonal & subclonal LOH/HD/Gain -----------------
...
ggsave(fp("SF19c_LOH_clonal_size_vs_reptime.pdf"),
       plot_reptime_clonality("SF19c", "Size of clonal LOH segment"),
       width = 80/25.4, height = 60/25.3); message("Saved: SF19c")
ggsave(fp("SF19d_HD_clonal_size_vs_reptime.pdf"),
       plot_reptime_clonality("SF19d", "Size of clonal HD segment"),
       width = 80/25.4, height = 60/25.3); message("Saved: SF19d")
ggsave(fp("SF19e_GAIN_clonal_size_vs_reptime.pdf"),
       plot_reptime_clonality("SF19e", "Size of clonal GAIN segment"),
       width = 80/25.4, height = 60/25.3); message("Saved: SF19e")
ggsave(fp("SF19f_LOH_subclonal_size_vs_reptime.pdf"),
       plot_reptime_clonality("SF19f", "Size of subclonal LOH segment"),
       width = 80/25.4, height = 60/25.3); message("Saved: SF19f")
ggsave(fp("SF19g_HD_subclonal_size_vs_reptime.pdf"),
       plot_reptime_clonality("SF19g", "Size of subclonal HD segment"),
       width = 80/25.4, height = 60/25.3); message("Saved: SF19g")
ggsave(fp("SF19h_GAIN_subclonal_size_vs_reptime.pdf"),
       plot_reptime_clonality("SF19h", "Size of subclonal GAIN segment"),
       width = 80/25.4, height = 60/25.3); message("Saved: SF19h")

# ---- SF22  Simulated progression — punctuated + gradual models -------------
sims_alt <- sd_tsv("SF22")
met_progression_data <- sd_tsv("Fig4b") %>% dplyr::filter(gs_grade == "Metastasis")
met_summary_data <- met_progression_data %>% group_by(trajectory) %>%
  dplyr::summarise(met_first_quartile = quantile(total_poete, .25),
                   met_median = median(total_poete),
...
ggsave(fp("SF22_simulated_progression_gg_alternative_models.pdf"), p_combined,
       width = 160/25.3, height = 100/25.3, dpi = 300)
message("Saved: SF22")

# ---- SF24a  Cox forest, MFS, SPOP wild-type subset -------------------------
met_relapse <- sd_tsv("SF24a") %>%
  mutate(high_pga = fct_relevel(high_pga, "Low"),
         advanced = fct_relevel(advanced, "Low"))
model <- coxph(Surv(time2relapse, relapse_ind) ~
                 ggroup + psa_cat + t_stage_simplified + age +
                 trajectory + mut_ith + nsubclones + advanced +
                 high_pga, data = met_relapse)
pdf(fp("SF24a_mfs_by_subclonal_dynamics_spop_wt.pdf"),
    height = 100/25.3, width = 120/25.3)
print(ggforest(model, data = as.data.frame(met_relapse))); dev.off()
message("Saved: SF24a")

# ---- SF24b Cox forest, MFS with TSG alteration burden -------------
met_relapse <- sd_tsv("SF24b") %>%
  mutate(high_pga = fct_relevel(high_pga, "Low"),
         advanced = fct_relevel(advanced, "Low"))
model <- coxph(Surv(time2relapse, relapse_ind) ~
              ggroup + psa_cat + t_stage_simplified + age +
            ...
            data = met_relapse)
pdf(fp("SF24b_mfs_by_subclonal_dynamics_clinical_and_genetic.pdf"),
    height = 100/25.3, width = 120/25.3)
print(ggforest(model, data = as.data.frame(met_relapse))); dev.off()
message("Saved: SF24b")

# ---- SF24c Cox forest, MFS with clinical + genetic covariates ---------------------
met_relapse <- sd_tsv("SF24c") %>%
  mutate(high_pga = fct_relevel(high_pga, "Low"),
         advanced = fct_relevel(advanced, "Low"))
...
model <- coxph(Surv(time2relapse, relapse_ind) ~
            ...
            data = met_relapse)
pdf(fp("SF24c_mfs_by_subclonal_dynamics_tsg_alt.pdf"),
    height = 100/25.3, width = 120/25.3)
print(ggforest(model, data = as.data.frame(met_relapse))); dev.off()
message("Saved: SF24c")

# ---- SF24d  KM: D'Amico risk + evolutionary risk ---------------------------
met_relapse <- sd_tsv("SF24d")
km_fit <- survfit(Surv(time2relapse, relapse_ind) ~ damico_evorisk, data = met_relapse)
pdf(fp("SF24d_km_evorisk_damico.pdf"), height = 80/25.3, width = 80/25.3)
...
# ---- SF24e  KM: Cambridge Prognostic Group + evolutionary risk -------------
met_relapse <- sd_tsv("SF24e")
km_fit <- survfit(Surv(time2relapse, relapse_ind) ~ cpg_evorisk, data = met_relapse)
pdf(fp("SF24e_km_evorisk_cpg.pdf"), height = 80/25.3, width = 80/25.3)
print(ggsurvplot(km_fit, data = met_relapse,
...
# ---- SF25b-g  TTF KM: Taxane vs ARPI, 2nd line ----------------------------
render_sf25 <- function(panel, filename) {
  data <- sd_tsv(panel)
...
render_sf25("SF25b", "SF25b_TTF_in_Taxane_vs_ARPI_2nd_line_O-I.pdf")
render_sf25("SF25c", "SF25c_TTF_in_Taxane_vs_ARPI_2nd_line_O-II.pdf")
render_sf25("SF25d", "SF25d_TTF_in_Taxane_vs_ARPI_2nd_line_O-III.pdf")
render_sf25("SF25e", "SF25e_TTF_in_Taxane_vs_ARPI_2nd_line_O-I_non_CIN.Cluster-6.pdf")
render_sf25("SF25f", "SF25f_TTF_in_Taxane_vs_ARPI_2nd_line_O-II_non_CIN.Cluster-6.pdf")
render_sf25("SF25g", "SF25g_TTF_in_Taxane_vs_ARPI_2nd_line_O-III_non_CIN.Cluster-6.pdf")


# =============================================================================
# 7. SUPPLEMENTARY FIGURES, part 2 (SF*, former Supplementary set)  -----------
# =============================================================================

# ---- SF2a  DPClust vs Phylogic CCF scatter, example PPCG0004a_DNA -----------
sample_data <- sd_tsv("SF2a")
...
ggsave(fp("SF2a_dpclust_vs_phylogic_ccf_scatter.pdf"), ccf_corr,
       height = 65/25.3, width = 65/25.3)
ggsave(fp("SF2a_dpclust_vs_phylogic_ccf_scatter.png"), ccf_corr,
       height = 65/25.3, width = 65/25.3)
message("Saved: SF2a")

# ---- SF2b  Histogram: Pearson CCF DPClust vs PhylogicNDT --------------------
dvp <- sd_tsv("SF2b")
ccf_hist <- ggplot(dvp, aes(x = pearson)) +
  ...
  xlim(c(0, 1.05))
ggsave(fp("SF2b_dpclust_vs_phylogic_ccf_pearson_histogram.pdf"), ccf_hist,
       width = 65/25.3, height = 65/25.3)
ggsave(fp("SF2b_dpclust_vs_phylogic_ccf_pearson_histogram.png"), ccf_hist,
       width = 65/25.3, height = 65/25.3)
message("Saved: SF2b")

# ---- SF2c  Heatmap: subclone count DPClust vs PhylogicNDT -------------------
heat_df <- sd_tsv("SF2c")
sf2c <- heat_df %>%
...
ggsave(fp("SF2c_scatterplot_dpclust_vs_phylogic_nsubclones.pdf"), sf2c,
       width = 65/25.3, height = 65/25.3)
ggsave(fp("SF2c_scatterplot_dpclust_vs_phylogic_nsubclones.png"), sf2c,
       width = 65/25.3, height = 65/25.3)
message("Saved: SF2c")

# ---- SF2d  DPClust vs Phylogic histogram, example PPCG1060a_DNA -------------
sample_data <- sd_tsv("SF2d")
sid <- unique(sample_data$sample)[1]
ari <- NA_real_
...
ggsave(fp("SF2d_dpclust_vs_phylogic_clusters.pdf"), combined,
       height = 90/25.3, width = 90/25.3)
ggsave(fp("SF2d_dpclust_vs_phylogic_clusters.png"), combined,
       height = 90/25.3, width = 90/25.3)
message("Saved: SF2d")

# ---- SF2e  ARI histogram across samples -------------------------------------
dvp <- sd_tsv("SF2e")
...
ggsave(fp("SF2e_dpclust_vs_phylogic_ari_histogram.pdf"), ari_hist,
       width = 65/25.3, height = 65/25.3)
ggsave(fp("SF2e_dpclust_vs_phylogic_ari_histogram.png"), ari_hist,
       width = 65/25.3, height = 65/25.3)
message("Saved: SF2e")

# ---- SF2f  ARI histogram stratified by subclonal PGA ------------------------
dvp <- sd_tsv("SF2f")
ari_hist <- dvp %>%
  ...
  theme(legend.position = "top")
ggsave(fp("SF2f_dpclust_vs_phylogic_ari_histogram_by_subclonal_pga_group.pdf"), ari_hist,
       height = 65/25.3, width = 65/25.3)
ggsave(fp("SF2f_dpclust_vs_phylogic_ari_histogram_by_subclonal_pga_group.png"), ari_hist,
       height = 65/25.3, width = 65/25.3)
message("Saved: SF2f")

# ---- SF2g  CCF scatter, clonal CN regions -----------------------------------
sf2g <- sd_tsv("SF2g")
ccf_clonal <- ggplot(sf2g, aes(x = truncated_ccf, y = phylogic_ccf)) +
  ...
  xlim(c(0, 1)) + ylim(c(0, 1))
ggsave(fp("SF2g_dpclust_vs_phylogic_ccf_scatter_clonal_cn.png"), ccf_clonal,
       height = 65/25.3, width = 65/25.3)
ggsave(fp("SF2g_dpclust_vs_phylogic_ccf_scatter_clonal_cn.pdf"), ccf_clonal,
       height = 65/25.3, width = 65/25.3)
message("Saved: SF2g")

# ---- SF2h  CCF scatter, subclonal CN regions --------------------------------
sf2h <- sd_tsv("SF2h")
ccf_sub <- ggplot(sf2h, aes(x = truncated_ccf, y = phylogic_ccf)) +
  geom_point(alpha = 0.01) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "lightgreen") +
  geom_smooth(method = "lm", color = "blue", se = FALSE) +
  ggpubr::stat_cor(method = "spearman", color = "blue", size = 2) +
  ...
  xlim(c(0, 1)) + ylim(c(0, 1))
ggsave(fp("SF2h_dpclust_vs_phylogic_ccf_scatter_subclonal_cn.png"), ccf_sub,
       height = 65/25.3, width = 65/25.3)
ggsave(fp("SF2h_dpclust_vs_phylogic_ccf_scatter_subclonal_cn.pdf"), ccf_sub,
       height = 65/25.3, width = 65/25.3)
message("Saved: SF2h")

# ---- SF4  Clock signature (SBS1/5/40) burden vs age -------------------------
df <- sd_tsv("SF4")
p1 <- plot_facet(df, "soft", "age_at_tumour_collection")
p2 <- plot_facet(df, "hard", "age_at_tumour_collection")
p <- (p1 / p2) + plot_annotation(tag_levels = "a")
save_ggplot(p, fp("SF4_clock_sbs2age"), w = 200, h = 100)
message("Saved: SF4")

# ---- SF5  MRCA latency cohort diagram ---------------------------------------
pt_rate_mrca_estimates <- sd_tsv("SF5")
p <- plot_cohort_diagram(pt_rate_mrca_estimates)
ggsave(fp("SF5_mrca_latency_cohort_diagram.pdf"), p,
       width = 180/25.3, height = 180/25.3)
message("Saved: SF5")

# ---- SF6  Global vs patient-specific clock rate -----------------------------
global_rate_mrca <- sd_tsv("SF6")
pt_rate_mrca     <- sd_tsv("SF5")
p1 <- merge(
...
save_ggplot(p, fp("SF6_global_vs_patient_rate"), w = 180, h = 250)
message("Saved: SF6")

# ---- SF7  MRCA / CNA latency distributions by Grade Group -------------------
sf7 <- sd_rds("SF7")
t_mrca <- sf7$mrca
earliest_cnas <- sf7$earliest_cnas
...
ggsave(fp("SF7_latency_by_grades.pdf"), p,
       width = 120/25.3, height = 120/25.3)
message("Saved: SF7")

# ---- SF8  Proliferation markers by Grade Group ------------------------------
sf8 <- sd_rds("SF8")
methylation_data <- sf8$methylation
gsea_rank        <- sf8$gsea_rank
...
p <- plot_grid(p1, p2, p3, p4, p5, NULL, nrow = 2, ncol = 3,
               ...
               labels = c("a)", "b)", "c)", "d)", "e)", ""))
ggsave(fp("SF8_proliferation_markers_by_grade_group.png"), p,
       width = 180/25.3, height = 120/25.3, dpi = 300)
ggsave(fp("SF8_proliferation_markers_by_grade_group.pdf"), p,
       width = 180/25.3, height = 120/25.3)
message("Saved: SF8")

# ---- SF9a  Externally generated (trajectory-derivation methodology) ---------
# See README.md for the companion repository link.

# ---- SF9b  Sankey: Woodcock et al. vs PPCG trajectory -----------------------
df_sankey <- sd_tsv("SF9b")
names(trajectory_colors) <- c("Canonical", "Alternative", "Gain-enriched")
...
p <- ggplot(df_sankey, aes(y = n, axis1 = woodcock_trajectory, axis2 = ppcg_trajectory)) +
  ...
  scale_fill_manual(values = trajectory_colors) +
  theme_void() + theme(legend.position = "none")
ggsave(fp("SF9b_sankey_woodcock_ppcg.pdf"), p,
       height = 50/25.4, width = 50/25.4, dpi = 300)
message("Saved: SF9b")

# ---- SF9c  Trajectory counts by country -------------------------------------
country_counts <- sd_tsv("SF9c")
p <- ggplot(country_counts, aes(x = country, y = n, fill = trajectory)) +
  ...
  scale_fill_manual(values = trajectory_colors)
ggsave(fp("SF9c_trajectory_counts_by_country.pdf"), p,
       width = 70/25.3, height = 50/25.3, dpi = 300)
message("Saved: SF9c")

# ---- SF10 (amps)  Genome-wide GISTIC amplification scores --------------------
amps <- sd_tsv("SF10_amps")
p <- amps %>%
...
p <- axes2lemon(p)
ggsave(fp("SF10_gistic_amps.pdf"), p,
       width = 100/25.3, height = 60/25.3, dpi = 300)
message("Saved: SF10 (amps)")

# ---- SF10 (dels)  Genome-wide GISTIC deletion scores -------------------------
dels <- sd_tsv("SF10_dels")
p <- dels %>%
  ggplot(aes(x = Start, y = G.score, color = trajectory)) +
  geom_line(alpha = 0.5) +
  scale_color_manual(values = trajectory_colors) +
  ...
  facet_wrap(~Chromosome, scales = "free_x", nrow = 1)
p <- axes2lemon(p)
ggsave(fp("SF10_gistic_dels.pdf"), p,
       width = 100/25.3, height = 60/25.3, dpi = 300)
message("Saved: SF10 (dels)")

# ---- SF11  Per-chromosome-arm GISTIC deletion scores -------------------------
...
if (file.exists(chr_sizes_fp)) {
  ...
  lp <- lapply(relevant_loss_arms, function(arm) {
    ...
    sub <- sf11 %>% dplyr::filter(Chromosome == chr_n,
  ...
  ggsave(fp("SF11_Del_chr_arms_gistic.pdf"), p,
         width = 180/25.3, height = 250/25.3, dpi = 300)
  message("Saved: SF11")
} else {
  message("Skipped SF11: cytoband file ", chr_sizes_fp, " not available")
}

# ---- SF12  Per-chromosome-arm GISTIC amplification scores -------------------
sf12 <- sd_tsv("SF12")
if (file.exists(chr_sizes_fp)) {
  ...
  lp <- lapply(relevant_gain_arms, function(arm) {
    ...
    if (nrow(row) == 0) return(NULL)
    chr_n <- gsub("chr", "", row$chr)
    sub <- sf12 %>% dplyr::filter(Chromosome == chr_n,
                                  Start <= row$end, End >= row$start) %>%
      dplyr::mutate(coordinate = (Start + End) / (2 * 1e6))
    p = ggplot(sub, aes(x = coordinate, y = G.score)) +
      geom_line(alpha = 0.5, aes(color = trajectory)) +
  ...
  lp <- lp[!sapply(lp, is.null)]
  p <- cowplot::plot_grid(plotlist = lp, ncol = 3)
  ggsave(fp("SF12_Amp_chr_arms_gistic.pdf"), p,
         width = 180/25.3, height = 250/25.3, dpi = 300)
  message("Saved: SF12")
} else {
  message("Skipped SF12: cytoband file ", chr_sizes_fp, " not available")
}

# ---- SF15a  TCGA molecular subtype donuts per trajectory --------------------
donut_data <- sd_tsv("SF15a")
...
ggsave(fp("SF15a_evotype_overlap_tcga_subtypes_donut.pdf"), p,
       width = 225/45.3, height = 100/25.3)
message("Saved: SF15a")

# ---- SF15b  You et al. expression subtypes per trajectory -------------------
exp_donut <- sd_tsv("SF15b")
p <- ggplot(exp_donut, aes(x = 2, y = proportion, fill = expression_subtype_you)) +
  ...
  geom_text(
    ...
    aes(x = 0.5, y = 0, label = paste0("N=", sample_count)),
    inherit.aes = FALSE, size = 3, fontface = "italic", color = "grey30")
ggsave(fp("SF15b_evotype_overlap_you_subtypes.pdf"), p,
       width = 100/25.3, height = 100/25.3)
message("Saved: SF15b")

# ---- SF17  Trajectory timelines (multi-panel) -------------------------------
timeline_data <- sd_tsv("SF17")
p3 <- plot_timeline_raincloud_tj(timeline_data)
p3 <- axes2lemon(p3)
# The original SF17 also includes p1 (MRCA latency per trajectory) and p2 (CNA
...
ggsave(fp("SF17_latency_by_trajectories.pdf"), p,
       width = 155/25.3, height = 120/25.3)
message("Saved: SF17")

# ---- SF20  Sankey: CombiMets primary -> metastasis trajectory transitions ---
pair_tj <- sd_tsv("SF20")
...
p <- ggplot(sankey_data,
  ...
  scale_fill_manual(values = c("1" = "#8DA0CB",
                               ...
                               "3" = "#C04667")) +
  theme(legend.position = "none")
ggsave(fp("SF20_tj_transitions_sankey.pdf"), p,
       width = 150/25.3, height = 100/25.3)
message("Saved: SF20")

# ---- SF21  Evolutionary rates by Gleason score ------------------------------
evo_metrics_rates <- sd_tsv("SF21") %>% dplyr::mutate(gs_grade = ifelse(gs_grade == "4+", "4-5", gs_grade))
...
p <- cowplot::plot_grid(p1, p2, p3, p4, nrow = 2,
                        labels = c("a)", "b)", "c)", "d)"))
ggsave(fp("SF21_evolutionary_rates_by_gs.pdf"), p,
       width = 130/25.4, height = 130/25.4, units = "in")
message("Saved: SF21")

# ---- SF23  Rate forest plots (3 Cox models) ---------------------------------
met_relapse <- sd_tsv("SF23") %>% as.data.frame()
...
pdf(fp("SF23_rate_forest_plots.pdf"), width = 120/25.3, height = 180/25.3)
print(combined_forest); dev.off()
message("Saved: SF23")

# =============================================================================
message("\nAll reproducible panels written to: ", fig_out_dir)
message("Schematics (Fig1b, Fig2a, Fig4c, Fig5b, SF25a) are not generated by code.")
message("External panels (SF16, SF9a) are produced by companion repos — see README.md.")
