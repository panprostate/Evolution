...
st1 = qc %>% dplyr::select(
    `PPCG Sample ID`                 = PPCG_Sample_ID,
    `PASS PPCG Sample QC`            = pass_sample_qc,
    `NOT in PPCG Sample Blacklist`   = pass_blacklist,
    `NRPCC`                          = nrpcc,
    `PASS NRPCC QC`                  = pass_nrpcc_qc,
    `Index sample in PPCG Evolution` = ppcg_evo_sel_one,
    `Final PASS label`               = PASS
)
fs::dir_create('outputs/00_preprocessing', recurse = TRUE)
data.table::fwrite(st1, 'outputs/00_preprocessing/ST1_sample_qc.csv')
