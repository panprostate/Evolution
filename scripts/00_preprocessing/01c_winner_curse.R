...
# SF1c - Relationship between NRPCC and ratio of subclonal mutations
write_tsv(
  muts_wcc_df %>% dplyr::filter(NRPCC >= 10) %>%
    dplyr::select(NRPCC, nclonal_wcc, nsubclonal_wcc),
  file.path(outdir, "SF1c_source_data.tsv")
)
ggsave(file.path(figdir, "SF1c_ratio_subclonal_mutations_wcc_vs_nrpcc.pdf"), width = 50/25.3, height = 50/25.3)
