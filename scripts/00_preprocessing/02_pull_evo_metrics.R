...
# Supplementary Table 2, with informative column names. nscnas (number of SCNAs)
# is dropped here; it is retained in evo_metrics.tsv above.
st2 = evo_df %>% dplyr::select(
    `Patient ID`                     = record_id,
    `Sample ID`                      = smp,
    `Number of subclones`            = nsubclones,
    `Fraction of subclonal mutations` = mut_ith,
    `Number of subclonal mutations`  = subclonal_burden,
    `CCF of largest subclone`        = largest_subclone,
    `CCF sum of subclones`           = ccf_sum_subclone,
    `Clonal PGA`                     = clonal_pga,
    `Subclonal PGA`                  = subclonal_pga,
    `Total PGA`                      = total_pga,
    `Grade Group`                    = group,
    `PSA`                            = psa_at_tumour_collection,
    `Pathological T stage`           = path_t_stage,
    `Age at tumour collection`       = age_at_tumour_collection,
    `Metastasis indicator`           = mets_ind,
    `Ancestry`                       = predicted_ancestry,
    `Follow-up (days)`               = donor_interval_of_last_followup
)
write_delim(st2, file.path(outdir, "ST2.tsv"), delim = "\t")